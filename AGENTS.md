...
>>>>>>> fd4af0ed1a (docs: proibir travessao e assinatura de IA no contexto dos agentes)
...
Docs accompany every code change: update affected README and JSDoc contracts together. English is the source of truth and the only language a change must update; `README.zh.md` files are no longer gated, and only explicit user invocation runs `dsh-translate-docs`. Current-state prose, one line per paragraph, one home per fact, and word budgets live in [docs/AGENTS.md](docs/AGENTS.md).
